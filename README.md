...
```
$ docker run --rm mplatform/mquery redoracle/docker_fail2ban:latest
...
docker run -d --name fail2ban --restart always \
  ...
  redoracle/docker_fail2ban:latest
```
...
This image still uses `iptables` to preserve backwards compatibility
